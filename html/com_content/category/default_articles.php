...
// This has had no work on it yet as it's only used for MBRRACE Reports, currently.
// BUT, if something else does need it, then the reports template will certainly NOT be what we
// need, and there's not clear way of selecting which templae to use on the admin form, so we need
// divert to the report template here if the 'page class' is being used:
if (!empty($this->params->get('pageclass_sfx')) && file_exists(__DIR__ . '/' . $this->params->get('pageclass_sfx') . '.php')) {
    require(__DIR__ . '/' . $this->params->get('pageclass_sfx') . '.php');
    return;
}
