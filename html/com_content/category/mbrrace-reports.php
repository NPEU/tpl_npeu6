...
// Filter the list:
$items = $this->items;
// No way around hard-coding this value, but better then looking for thr group name which could
// change:
...
foreach ($items as $i => $article) {
    if (!empty($article->jcfields)) {
        ...
        foreach ($fields as $field) {
            #echo '<pre>'; var_dump($field->group_id == $report_field_group_id); echo '</pre>';
            #echo '<pre>'; var_dump(in_array($field->value, $report_filters)); echo '</pre>';
            if ($field->group_id == $report_field_group_id && in_array($field->value, $report_filters)) {
                $found = true;
                break;
            }
        }
